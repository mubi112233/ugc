...
export default function CaseStudyClient() {
  ...
  const currentLang = rawLang === "de" || rawLang === "ge" ? "ge" : "en";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchApiDataClient<any>(API_ENDPOINTS.CASE_STUDIES, apiLang);
        const caseStudies: CaseStudyData[] = Array.isArray(data?.caseStudies) ? data.caseStudies : [];
        const found = caseStudies.find((cs) => cs.caseStudyId === caseStudyId);
        if (!found) {
          setNotFound(true);
        } else {
          setCaseStudy(found);
    ...
    return (
      <div className={`min-h-screen ${SPACING.sideMargin} bg-background flex flex-col items-center justify-center gap-4`}>
        <p className="text-muted-foreground text-lg">
          {currentLang === "ge" ? "Fallstudie nicht gefunden." : "Case study not found."}
        </p>
        <Link
        ...
        >
          ...
          {currentLang === "ge" ? "Zurück zu Fallstudien" : "Back to Case Studies"}
        </Link>
      </div>
    );
  }
      ...
      <article className="max-w-5xl mx-auto py-6 sm:py-8 md:py-12 px-4 sm:px-6 lg:px-8">
          ...
          <section className="mb-10 sm:mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              {caseStudy.results.map(
                (result: { value: string; metric: string; description: string }, index: number) => (
                  <div
                    key={index}
                    className="bg-gradient-to-br from-card to-card/50 backdrop-blur-sm border border-border/50 rounded-xl sm:rounded-2xl p-5 sm:p-6 hover:border-gold/30 transition-all hover:shadow-lg hover:scale-105"
                  >
        ...
        {caseStudy.testimonial && (
          <aside className="bg-gradient-to-br from-gold/10 via-gold/5 to-transparent backdrop-blur-sm border border-gold/20 rounded-xl sm:rounded-2xl p-6 sm:p-8 lg:p-10 mb-10 sm:mb-12 shadow-lg">
            <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4 mb-4 sm:mb-6">
              <svg
                className="w-10 h-10 sm:w-12 sm:h-12 text-gold/30 flex-shrink-0"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        <footer className="mt-12 sm:mt-16 pt-6 sm:pt-8 border-t border-border/50 text-center">
          ...
          >
            ...
            <span className="hidden sm:inline">
              {currentLang === "ge" ? "Alle Fallstudien ansehen" : "View All Case Studies"}
            </span>
            <span className="sm:hidden">
              {currentLang === "ge" ? "Alle Fallstudien" : "All Studies"}
            </span>
          </Link>
        </footer>
      </article>
