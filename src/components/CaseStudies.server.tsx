...
import { fetchCaseStudies, type CaseStudiesResponse } from "@/lib/api";
...
export function CaseStudies({ lang }: { lang: string }) {
  ...
  useEffect(() => {
    fetchCaseStudies(currentLang).then((data: CaseStudiesResponse | null) => {
      if (!Array.isArray(data?.caseStudies)) return;
      const mapped = (data.caseStudies as any[])
        .map((cs) => ({
      ...
      <div className={`container mx-auto ${SPACING.container} relative z-10`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-7xl mx-auto">
          {studies.map((study) => (
            <Link
              ...
              className="group bg-card border border-border/50 rounded-xl sm:rounded-2xl overflow-hidden hover:border-brand/50 hover:shadow-[0_20px_60px_-15px_hsl(142_70%_45%/0.3)] transition-all duration-300 hover:-translate-y-2 w-full block"
            >
              ...
              <div className="p-4 sm:p-5 lg:p-6">
                ...
                <h3 className="text-base sm:text-lg lg:text-xl font-bold mb-2 sm:mb-3 text-foreground group-hover:text-brand transition-colors line-clamp-2">
                  {study.title}
                </h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-3 text-xs text-muted-foreground">
                    <span className="text-brand font-semibold">{study.stats?.costSaved}</span>
                    <span>{copy.labels.saved}</span>
                  </div>
                  <div className="flex items-center gap-1 sm:gap-2 text-brand font-semibold text-xs sm:text-sm group-hover:gap-2 sm:group-hover:gap-3 transition-all">
        ...
        <div className="mt-12 sm:mt-16 lg:mt-20 text-center">
          ...
          <Link
            ...
            className="inline-block w-full sm:w-auto px-8 sm:px-10 py-4 sm:py-5 bg-gradient-to-r from-brand to-gold text-foreground font-bold text-base sm:text-lg rounded-2xl hover:from-gold hover:to-secondary transition-all duration-300 hover:scale-105 shadow-xl hover:shadow-2xl text-center"
          >
